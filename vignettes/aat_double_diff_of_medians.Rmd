...
## Sets function
The sets function receives data from a single participant and returns a list of datasets for each condition. In this case, we will generate four lists, containing the trials from: avoid test, approach test, avoid control, and approach control.
```
...
## Score function
The score function receives these four lists from a single participant and for each: 
...
```
aat_splits <- sh_apply(ds_aat, "UserID", aat_fn_sets, aat_fn_score, 10000)
```
