...
## Preprocessing
Only select trials from assessment blocks, using the first response of the first attempt at each trial
```
...
## Sets Function
The sets function receives data from a single participation and returns a list of datasets for each condition. In this case, we will generate four lists, containing the trials from: avoid test, approach test, avoid control, and approach control
```
...
## Score Function
The score function receives these four lists from a single participation and for each: 
