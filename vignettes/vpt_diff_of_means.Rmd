...
## Sets function
The sets function receives data from a single participant and returns a list of datasets for each condition. In this case, we will generate two lists: one with data from trials with probe-at-test (patt == "yes") and one with data from trials with probe-at-control (patt == "no"). 
```
...
## Score function
The score function receives these two lists from a single participant and for each: 
