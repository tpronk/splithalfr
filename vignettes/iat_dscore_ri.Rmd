...
```
The improved d-score algorithm specifies that participations whose RTs for over 10% of reponses are below < 300 ms should be dropped. The R-script below identifies such participations. Note that none of the participations met this criteria, so all can be kept for successive analyses.
```
ds_summary <- ds_iat %>%
